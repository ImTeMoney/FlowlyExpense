    ...
    <div className="debt-tracker-card glass-card">
      ...
      {showAdd && (
        <form className="debt-add-form" onSubmit={handleAdd}>
          <div className="debt-direction-toggle">
            <button
              type="button"
              className={`debt-dir-btn${form.direction === 'owes_me' ? ' active-owes' : ''}`}
              onClick={() => setForm(f => ({ ...f, direction: 'owes_me' }))}
            >{he ? 'חייבים לי' : 'Owes me'}</button>
            <button
              type="button"
              className={`debt-dir-btn${form.direction === 'i_owe' ? ' active-iowe' : ''}`}
              onClick={() => setForm(f => ({ ...f, direction: 'i_owe' }))}
            >{he ? 'אני חייב' : 'I owe'}</button>
          </div>
      ...
      {active.length > 0 && (
        <ul className="debt-list">
          {active.map(d => (
            <li key={d.id} className={`debt-item${d.direction === 'owes_me' ? ' owes-me' : ' i-owe'}`}>
              ...
              <button className="debt-delete-btn" onClick={() => dispatch({ type: 'DELETE_DEBT', payload: d.id })}
                title={he ? 'מחק' : 'Delete'}>
                <Trash2 size={13} />
              </button>
            </li>
